...
__device__ void warpReduce(float * data, int tid){
    float sum = data[tid] + data[tid+32];
...
__global__ void reduce(float * d_input, float * d_output){
    ...
    // 因此应该*2.
    sdata[tid] = d_input[i] + d_input[i+blockDim.x];
...
int main(){
    ...
    reduce<N><<<grid,block>>>(d_input, d_output);
