...
Usage:
  ...
  bash run_ncu.sh KERNEL [REPORT_BASE]   # backward-compatible form

Options:
  -k, --kernel NAME          pybind kernel name (default: sgemm_tf32_bt)
...
The script first builds the extension and validates the pybind name. New kernels
using lib.kernel(a, b, c) work without editing this script. If the CUDA global
function name does not contain the pybind name, pass --kernel-filter explicitly.
EOF
}
...
if [[ "${kernel_filter}" == "auto" ]]; then
    if [[ "${kernel_name}" == sgemm_cublas* ]]; then
